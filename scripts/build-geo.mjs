...
// Texas data is emitted as one module PER CONCERN (outline / polygon /
// districts / metros), not one monolith: bundler code-splitting is
// module-granular, and the TuxChartGeographic kind children each import
// only what they render — a `kind="flow"` consumer must not download
// the 110KB district geometry just to get the 8KB outline.
const texasOutlineOut = `${HEADER}
...
`;
fs.writeFileSync(path.join(OUT, "texas-outline.ts"), texasOutlineOut);

const texasPolygonOut = `${HEADER}
...
`;
fs.writeFileSync(path.join(OUT, "texas-outline-polygon.ts"), texasPolygonOut);

const districtsOut = `${HEADER}
...
`;
fs.writeFileSync(path.join(OUT, "txdot-districts.ts"), districtsOut);

const metrosOut = `${HEADER}
...
`;
fs.writeFileSync(path.join(OUT, "tx-metros.ts"), metrosOut);

// Compatibility barrel — importing it statically pulls all four modules.
const texasBarrel = `${HEADER}
...
`;
fs.writeFileSync(path.join(OUT, "texas.ts"), texasBarrel);
...
console.log("→ done");
console.log(`  texas-outline.ts:         ${size(path.join(OUT, "texas-outline.ts"))} KB`);
console.log(`  texas-outline-polygon.ts: ${size(path.join(OUT, "texas-outline-polygon.ts"))} KB`);
console.log(`  txdot-districts.ts:       ${size(path.join(OUT, "txdot-districts.ts"))} KB`);
console.log(`  tx-metros.ts:             ${size(path.join(OUT, "tx-metros.ts"))} KB`);
console.log(`  texas.ts (barrel):        ${size(path.join(OUT, "texas.ts"))} KB`);
